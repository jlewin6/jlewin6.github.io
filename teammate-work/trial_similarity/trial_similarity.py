...
def similarity_score(input):
    ...
    df_sim_score = pd.DataFrame({'similarity_score_title': np.sort(A[0])[-(top_n+1):-1]})
    df_sim_score_2 = pd.DataFrame({'similarity_score_description': np.sort(A_2[0])[-(top_n+1):-1]})
